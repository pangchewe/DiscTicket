...
module.exports = class extends Command {
	constructor(...args) {
		super(...args, {
			...
			description: "Set the applications ticket category.",
	...
	async run(message, args, { settings }) {
		...
		if (value.toLowerCase() === "none") {
			...
			return settings
				.save()
				.then(() => message.channel.send({ embeds: [this.client.embed.success(`Removed application category.`)] }))
		...
		settings
			.save()
			.then(() => message.channel.send({ embeds: [this.client.embed.success(`Set application category to \`${category.name}\`.`)] }))
			.catch((err) => message.channel.send({ embeds: [this.client.embed.error(`Failed to save settings: \`${err.message}\`.`)] }));
	}
};
