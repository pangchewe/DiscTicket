...
module.exports = class extends Command {
	...
	async run(message, args, { settings }) {
		...
		if (value.toLowerCase() === "none") {
			...
			return settings
				.save()
				.then(() => message.channel.send({ embeds: [this.client.embed.success(`Removed review channel.`)] }))
		...
		settings
			.save()
			.then(() => message.channel.send({ embeds: [this.client.embed.success(`Set review channel to \`${channel.name}\`.`)] }))
			.catch((err) => message.channel.send({ embeds: [this.client.embed.error(`Failed to save settings: \`${err.message}\`.`)] }));
	}
};
